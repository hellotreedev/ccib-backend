					...
					<tr>
						...
						@endforeach
							@php
								...
								$appends_to_sort_query .= 'sort_by=created_at&';
                            ...
                            @endphp
						<th><a {!! $page['server_side_pagination'] ? 'href="' . url(config('hellotree.cms_route_prefix') . '/' . $page['route'] . $appends_to_sort_query) . '"' : '' !!}>{{ str_replace(['_id', '_'], ['', ' '], 'Created At') }}</a>
									@if ($page['server_side_pagination'])
									<div class="{{ 1 ? 'sort-arrows' : '' }} position-relative d-inline {{ request('sort_by') == 'created_at' ? request('sort_by_direction') : '' }}"></div>
									@endif
						</th>
						<th>Actions</th>
					</tr>
