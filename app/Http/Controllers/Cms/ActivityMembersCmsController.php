...
class ActivityMembersCmsController extends Controller
{
    ...
    public function index($route = "activity-members")
    {
    ...
    public function create($route = "activity-members")
    {
    ...
    public function store(Request $request, $route = "activity-members")
    {
        ...
        if (isset($request->socials)) {
            $request->validate([
                'socials.*.icon' => 'required',
                'socials.*.url' => 'required | starts_with:https://,http://',
            ]);
        };
        ...
        if (isset($request->socials)) {
            $i = 0;
            foreach ($request->socials as $single) {
            
                $social = new MemberSocialMedia();
                $social->url = $single['url'];
                $name = $single['icon']->getClientOriginalName();
                $social->icon = $single['icon']->storeAs('activity-member-social' . '/' . Str::uuid(), $name);
                $social->activity_members_id = $row->id;
                $social->ht_pos = $i + 1;
                $social->save();
                $i++;
            }
    ...
    public function show($id, $route = "activity-members")
    {
    ...
    public function edit($id, $route = "activity-members")
    {
    ...
    public function update(Request $request, $id, $route = "activity-members")
    {
        $page = CmsPage::where('route', $route)->when(request()->get('admin')['admin_role_id'], function ($query) { $query->where('edit', 1); })->firstOrFail();
        $page_fields = json_decode($page['fields'], true);
        $page_translatable_fields = json_decode($page['translatable_fields'], true);
        ...
        if (isset($request->socials)) {
            $request->validate([
                'socials.*.icon' => 'required_without:socials.*.prev_id|required_with:remove_file_socials.*.icon',
                'socials.*.url' => 'required | starts_with:https://,http://',
            ]);
        }
        ...
        if (isset($request->socials)) {
            if ($row->socials->count() > 0) {
                foreach ($row->socials as $singleExisting) {
                    $x = 0;
                    foreach ($request->socials as $single) {
                        if (isset($single['prev_id']) && $single['prev_id'] == $singleExisting['id']) {
                            $x++;
                        }
                    }
                    if ($x == 0) {
                        $singleExisting->delete();
                    }
                }
            }
            $i = 0;
            foreach ($request->socials as $single) {
                if (isset($single['prev_id'])) {
                    $social = MemberSocialMedia::where('id', $single['prev_id'])->first();
                    $social->url = $single['url'];
                    $social->ht_pos = $i + 1;
                    $social->activity_members_id = $row->id;
                    if (isset($single['icon'])) {
                        $name = $single['icon']->getClientOriginalName();
                        $social->icon = $single['icon']->storeAs('activity-members-social-media' . '/' . Str::uuid(), $name);
                    }
                    $social->save();
                } else {
                    $social = new MemberSocialMedia();
    ...
    public function uploadImages(Request $request, $route = "activity-members")
    {
        $files = [];
        if ($request['images']) {
    ...
    public function destroy($id, $route = "activity-members")
    {
    ...
    public function order($route = "activity-members")
    {
    ...
    public function changeOrder(Request $request, $route = "activity-members")
    {
        $page = CmsPage::where('route', $route)->firstOrFail();
        $model = 'App\\' . $page['model_name'];
    ...
    public function uploadFile($file, $route = "activity-members")
    {
        $path = null;
